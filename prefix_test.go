...
func TestIpamer_AcquireSpecificIP(t *testing.T) {
	...
	testWithBackends(t, func(t *testing.T, ipam *ipamer) {
		...
		require.NoError(t, err)
		require.Equal(t, uint64(254), prefix.availableips())
		// network and broadcast are reserved
		require.Equal(t, uint64(0), prefix.acquiredips())
		...
		prefix, err = ipam.NewPrefix(ctx, "2001:0db8:85a3::/120")
		require.NoError(t, err)
		require.Equal(t, uint64(255), prefix.availableips())
		// network is blocked
		require.Equal(t, uint64(0), prefix.acquiredips())
...
func TestIpamer_AcquireChildPrefixCounts(t *testing.T) {
	...
	testWithBackends(t, func(t *testing.T, ipam *ipamer) {
		...
		usage := prefix.Usage()
		require.Equal(t, "ip:0/4094", usage.String())
		...
		usage = prefix.Usage()
		require.Equal(t, "ip:0/4094 prefixes alloc:1 avail:768", usage.String())
