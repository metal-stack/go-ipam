...
type Ipamer interface {
	...
	// AcquireIPs will attempt to acquire many IPs from this Prefix.
	AcquireIPs(prefixCidr string, count uint64) ([]*IP, error)
	...
	// ReleaseIPsFromPrefix will release the given IP array from prefix for later usage.
	// If the Prefix or any IP are not found an NotFoundError is returned.
	ReleaseIPsFromPrefix(prefixCidr string, ips []string) error
